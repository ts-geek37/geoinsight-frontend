...
const StoreDetailsPanel: React.FC<StoreDetailsPanelProps> = ({
  ...
  const { data, error, isLoading, isFetching } = useGetStoreDetailsQuery(storeId);
  ...
  const { store, kpis, revenueTrend, transactionsTrend, demographics, poiSummary } = data;
      ...
      <div className="flex flex-col gap-3 p-4 border-b border-border bg-background/95 backdrop-blur-sm sticky top-0 z-10">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            {store?.rfmSegment && (
              <SegmentBadge segment={store.rfmSegment ?? SEGMENT_ENUM.CHAMPION} />
            )}
          ...
          {revenueTrend?.length > 0 && (
            <CardSection title="Revenue Trend">
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={revenueTrend} margin={{ top: 8, right: 16, bottom: 8, left: 0 }}>
                  ...
                  <Tooltip
                    ...
                    formatter={(v: number) => [`₹${(v / 1_00_000).toFixed(2)}L`, "Revenue"]}
                  />
                  ...
                  <Line
                    ...
                    dataKey="revenue"
                    stroke={
                      kpis?.[0]?.trend.isPositive === false ? "#ef4444" : "var(--segment-champion)"
                    }
                    strokeWidth={2}
                    dot={false}
                    activeDot={{ r: 4 }}
          ...
          {poiSummary?.length > 0 && (
            <CardSection title="POI Summary (3km radius)" hasHeight={false}>
              <div className="grid grid-cols-4 gap-3">
                {poiSummary.map((item) => {
                  const Icon = poiIconMap[item.label as keyof typeof poiIconMap];
                  return (
                    <div
                      key={item.label}
